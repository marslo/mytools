...
#    LastChange : 2026-08-07 23:38:18
...
#         Usage : jenkins-libs.sh [--lts] [--ln] [--dryrun]
...
# -------------------------------- constants ---------------------------------
declare -r  JENKINS_ROOT='/opt/jenkins'
declare -r  PLUGIN_ROOT="${JENKINS_ROOT}/plugins"
declare -r  GROOVY_LIB="${HOME}/.groovy/lib"
declare -r  LOG_ROOT="${JENKINS_ROOT}/logs"
...
# @credit: https://github.com/ppo/bash-colors  (output style matches groovy-libs.sh)
...
# highlight key tokens for the terminal only (versions/names green, paths/urls blue,
# connective words italic). classifies whole tokens → no nested-ANSI; LOG_FILE never sees this.
function hl() {
  local s="${*}" indent rest
  ...
  for tok in "${toks[@]}"; do
    case "${tok}" in
      ...
      *                         ) out+="${_CI}${tok}${_CR} " ;;  # italic body
    esac
  done
  printf '%s%s' "${indent}" "${out% }"
}

# terminal gets a colored badge + highlighted body; LOG_FILE stays plain (no color)
# log to stderr and, once LOG_FILE is set, append to it (never aborts the run)
function log() {
...
function ucMeta() {
  if test 'lts' = "${WAR_CHANNEL}"; then printf 'stable'; else printf 'current'; fi
}
...
function usage() {
  command cat >&2 <<EOF
usage: jenkins-libs.sh [--lts] [--ln] [--dryrun]
  --lts        download the latest Jenkins LTS war (default: weekly)
  --ln         refresh ${GROOVY_LIB}/ (created if missing):
               prune dangling ${JENKINS_ROOT} links, then symlink core war + plugin *.jar
  --dryrun     print planned actions only; download/extract/link nothing
...
function latestCore() {
  local ver
  ver="$( command curl -fsSL --max-time 30 "${UC_BASE}/$( ucMeta )/latestCore.txt" 2>/dev/null \
          | command tr -d '[:space:]' )" || true
  printf '%s' "${ver}"
}
...
function installWar() {
  ...
  if test -d "${dest}/WEB-INF"; then
    ...
    command curl -fL --progress-bar --max-time 1800 -o "${tmp}" \
            "${UC_BASE}/download/war/${ver}/jenkins.war"
...
# -------------------------------- plugins -----------------------------------
# fetch update-center.actual.json once into UC_JSON (cache in the caller's shell)
# must be called outside command substitution so the assignment persists
function ensureUcJson() {
  if test -z "${UC_JSON}"; then
    ...
    command curl -fsSL --max-time 90 -o "${UC_JSON}" \
            "${UC_BASE}/$( ucMeta )/update-center.actual.json" \
      || die 'cannot fetch update-center.actual.json'
  fi
}
...
# these accumulate on upgrade: version-stamped jar names (jenkins-core-<ver>.jar
# ...) change, so old links dangle once `latest` re-points.
...
function installPlugin() {
  ...
  if test -d "${dest}/WEB-INF"; then
    ...
    if test 'download' = "${source}"; then
      ...
      command curl -fL --progress-bar --max-time 600 -o "${tmp}" \
              "${UC_BASE}/download/plugins/${artifact}/${ver}/${artifact}.hpi"
      src="${tmp}"
    fi
...
function main() {
  ...
  if test 'true' = "${DRY_RUN}"; then log '=== dryrun: no changes will be made ==='; fi
  ...
  if test 'true' = "${DO_LINK}"; then relinkLibs; fi
